 ...
 */
class BlockSubtractor:public IRollableSurfaceCalculator {
  private:
   ...
   * columns.
   * 
   * @param IRollableSurface*  The surface to test for visibility.
   * @param int  The X location of the tile to test in the surface.
   * @param int  The Y location of the tile to test in the surface.
   * @param std::vector<IRollableSurfaceProvider*>  The ordered columns.
   * @param bool  True if the columns have priority over the specified surface,
   *              otherwise false.
   * @param IRollableSurface::FaceDirection  Direction the surface being tested
   *           is facing.
   ...
   */
  bool isSurfaceTileVisible(IRollableSurface*, int, int, std::vector<IRollableSurfaceProvider*>, bool, IRollableSurface::FaceDirection, bool);
   ...
   * @returns  True if the surface tile is visible, otherwise false.
   */
  bool isSurfaceTileVisible(IRollableSurfaceProvider*, IRollableSurface*, int x, int y, IRollableSurface::FaceDirection);

  /**
   * Test whether the specified 2D location is vertically aligned with any of
   * the surfaces in the specified vector.
   * 
   * @param std::vector<IRollableSurface*>  TODO:
   * @param int  TODO:
   * @param int  TODO:
   * @returns true if TODO:
   */
  bool inSurface(std::vector<IRollableSurface*>, int, int);
   ...
   * 
   * @param IRollableSurfaceProvider*  The provider to generate the wall
   *           column from.
   ...
   */
  WallColumn* getRawWallColumn(IRollableSurfaceProvider*, int, int, IWallSurface::FaceDirection);
   ...
   * @returns  The completely raw unprocessed wall column.
   */
  std::vector<WallColumn*> getPhysicalWallColumn(IRollableSurfaceProvider*, int, int, IWallSurface::FaceDirection);
   ...
   * @returns  The completely raw unprocessed wall column.
   */
  std::vector<WallColumn*> getOptimisedWallColumn(IRollableSurfaceProvider*, int, int, IWallSurface::FaceDirection);

  IRollableSurface* getSurfaceAt(std::vector<IRollableSurface*>, int, int);
