...
void BlockSubtractor::registerRollableSurfaceProvider(IRollableSurfaceProvider* provider) {
  cCache.add(provider);
}

void BlockSubtractor::unregisterRollableSurfaceProvider(IRollableSurfaceProvider* provider) {
...
IRollableSurface* BlockSubtractor::getSurfaceAt(std::vector<IRollableSurface*> surfaces, int x, int y) {
...
bool BlockSubtractor::isSurfaceTileVisible(IRollableSurface* surface, int x, int y, std::vector<IRollableSurfaceProvider*> columns, bool columnHasPriority, IRollableSurface::FaceDirection facing, bool isTileHidden) {
  bool mIsTileHidden = isTileHidden;
  bool mIsTopSurface = facing == IRollableSurface::UP;
  ...
  for (unsigned int i = 0; i < columns.size(); i++) {
    ...
    if (mBlockArea->alligned(x, y)) {
      std::vector<IRollableSurface*> mTopSurfaces = columns[i]->getRollableSurfaces(IRollableSurface::UP);
      std::vector<IRollableSurface*> mBottomSurfaces = columns[i]->getRollableSurfaces(IRollableSurface::DOWN);
      IRollableSurface* mTopSurface = getSurfaceAt(mTopSurfaces, x, y);
      IRollableSurface* mBottomSurface = getSurfaceAt(mBottomSurfaces, x, y);
...
bool BlockSubtractor::isSurfaceTileVisible(IRollableSurfaceProvider* provider, IRollableSurface* surface, int x, int y, IRollableSurface::FaceDirection facing) {
  // TODO: Cache needs to know what it's relating to (e.g. the IRollableSurfaceProvider) to determine priority
  std::vector<IRollableSurface*> mTopSurfaces = provider->getRollableSurfaces(IRollableSurface::UP);
  std::vector<IRollableSurface*> mBottomSurfaces = provider->getRollableSurfaces(IRollableSurface::DOWN);
  IRollableSurface* mTopSurface = getSurfaceAt(mTopSurfaces, x, y);
  IRollableSurface* mBottomSurface = getSurfaceAt(mBottomSurfaces, x, y);
  int mTopHeight = mTopSurface->getSurfaceCellHeight(x, y);
  int mBottomHeight = mBottomSurface->getSurfaceCellHeight(x, y);
  bool mIsSubtraction = mBottomHeight > mTopHeight;
  std::vector<IRollableSurfaceProvider*> mSurfaceColumnsWithoutPriority = cCache.getSurfaceProviders(false, provider);
  if (!isSurfaceTileVisible(surface, x, y, mSurfaceColumnsWithoutPriority, false, facing, mIsSubtraction) != mIsSubtraction) {
    return false;
  }
  std::vector<IRollableSurfaceProvider*> mSurfaceColumnsWithPriority = cCache.getSurfaceProviders(true, provider);
  return isSurfaceTileVisible(surface, x, y, mSurfaceColumnsWithPriority, true, facing, false);
}

bool BlockSubtractor::inSurface(std::vector<IRollableSurface*> surfaces, int x, int y) {
  for (unsigned int i = 0; i < surfaces.size(); i++) {
...
int BlockSubtractor::getCompleteRows(IRollableSurfaceProvider* provider, std::vector<IRollableSurface*> calculatedSurfaces, IRollableSurface* surface, int west, int east, int south, IRollableSurface::FaceDirection facing/*, Condition* condition TODO:CONDITIONAL*/) {
...
std::vector<IRollableSurface*> BlockSubtractor::getRollableSurfaces(IRollableSurfaceProvider* provider, IRollableSurface::FaceDirection faceDirection) {
  ...
  std::vector<IRollableSurface*> mCalculatedSurfaces;
  IRollableSurface* mPreviousSurface = NULL;
  for (int y = mBlockCoverage->getSouth(); y <= mBlockCoverage->getNorth(); y++) {
    for (int x = mBlockCoverage->getWest(); x <= mBlockCoverage->getEast(); x++) {
      IRollableSurface* mSurface = getSurfaceAt(mRollableSurfaces, x, y);
...
WallColumn* BlockSubtractor::getRawWallColumn(IRollableSurfaceProvider* provider, int x, int y, IWallSurface::FaceDirection facing) {
...
std::vector<WallColumn*> BlockSubtractor::getPhysicalWallColumn(IRollableSurfaceProvider* provider, int x, int y, IWallSurface::FaceDirection facing) {
  ...
  std::vector<IRollableSurfaceProvider*> mSurfaceProviders = cCache.getSurfaceProviders(true, provider);
...
std::vector<WallColumn*> BlockSubtractor::getOptimisedWallColumn(IRollableSurfaceProvider* provider, int x, int y, IWallSurface::FaceDirection facing) {
  ...
  std::vector<IRollableSurfaceProvider*> mSurfaceProviders = cCache.getSurfaceProviders();
...
std::vector<IWallSurface*> BlockSubtractor::getWallSurfaces(IRollableSurfaceProvider* provider, IWallSurface::FaceDirection facing) {
