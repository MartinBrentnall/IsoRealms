...
void StateTile::notifyContact() {
  ...
  IStateTileFactory* mTileFactory = getElementFactory();
  mTileFactory->notifyContact(cState);
  cTheme->executeContactScript();
  mTileFactory->executeContactScript();
...
std::vector<IVisualElement*> StateTile::getVisualElements() {
  ...
  IStateTileFactory* mFactory = getElementFactory();
  if (mFactory->canChange(*cState)) {
