...
C64SpindizzySpriteSet::C64SpindizzySpriteSet(IRuntimeContext* runtimeContext, const std::string& name) {
  initialise();
  addTextures(runtimeContext, name);
  generateTextures();
}

void C64SpindizzySpriteSet::initialise() {
...
void C64SpindizzySpriteSet::resourceChanged(IColour* colour) {
  if (colour == cColour1 || colour == cColour2 || colour == cColour3 || colour == cOutlineColour) {
    generateTextures();
  }
}

void C64SpindizzySpriteSet::resourcePendingDestruction(IColour* destroyee, IColour* replacement) {
  bool mChanged = false;
...
void C64SpindizzySpriteSet::initialiseResource(DOMNodeWrapper* node, IResourceAccessor* resourceAccessor) {
...
void C64SpindizzySpriteSet::save(DOMNodeWriter* node, IResourceLocator* resourceLocator) {
...
IColour* C64SpindizzySpriteSet::getColour1() {
  return cColour1;
}

IColour* C64SpindizzySpriteSet::getColour2() {
  return cColour2;
}

IColour* C64SpindizzySpriteSet::getColour3() {
  return cColour3;
}

IColour* C64SpindizzySpriteSet::getColourOutline() {
  return cOutlineColour;
}

void C64SpindizzySpriteSet::setColour1(IColour* colour) {
  cColour1 = colour;
  generateTextures();
}

void C64SpindizzySpriteSet::setColour2(IColour* colour) {
  cColour2 = colour;
  generateTextures();
}

void C64SpindizzySpriteSet::setColour3(IColour* colour) {
  cColour3 = colour;
  generateTextures();
}

void C64SpindizzySpriteSet::setColourOutline(IColour* colour) {
  cOutlineColour = colour;
  generateTextures();
}

std::vector<ITexture*> C64SpindizzySpriteSet::getTextures() {
...
bool C64SpindizzySpriteSet::contains(ITexture* texture) {
  for (std::map<std::string, Texture*>::iterator i = cTextures.begin(); i != cTextures.end(); i++) {
    if (texture == i->second) {
      return true;
    }
...
void C64SpindizzySpriteSet::addTextures(IRuntimeContext* runtimeContext, const std::string& name) {
  for (std::map<std::string, Texture*>::iterator i = cTextures.begin(); i != cTextures.end(); i++) {
    runtimeContext->add(i->second, name + "_" + i->first);
  }
}

void C64SpindizzySpriteSet::deleteResources(IResourceInstanceListener<ITexture>* listener) {
  for (std::map<std::string, Texture*>::iterator i = cTextures.begin(); i != cTextures.end(); i++) {
