#include "SpindizzyCraftPyramidModels.h"

void SpindizzyCraftPyramidModels::createResources(DOMNodeWrapper* node, IRuntimeContext* runtimeContext) {
  for (int i = 0; i < node->getChildCount(); i++) {
    DOMNodeWrapper *mNode = node->getChild(i);
    std::string mValueAsString = mNode->getNodeName();
    if (mValueAsString == "ModelType") {
      SpindizzyCraftPyramidModelFactory* mResource = new SpindizzyCraftPyramidModelFactory();
...
void SpindizzyCraftPyramidModels::save(DOMNodeWriter* node, IResourceLocator* resourceLocator) {
...
extern "C" IPlugin* create() {
  return new SpindizzyCraftPyramidModels();
}
