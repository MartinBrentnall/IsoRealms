...
SpindizzyBlockType::SpindizzyBlockType(ISpindizzyBlockSet* elementSet) : ISpindizzyBlockType(elementSet) {
...
void SpindizzyBlockType::initialiseResource(DOMNodeWrapper* node, IResourceAccessor* resourceAccessor) {
...
IElement* SpindizzyBlockType::getElement(DOMNodeWrapper* node, BlockLocation* zoneLocation, IElementContainer* container) {
  ...
  SpindizzyBlock* mLoadedBlock = createBlock(&mStartLocation, &mEndLocation, cBlockProperties, mAddition);
  cContent.push_back(mLoadedBlock);
  ISpindizzyBlockSet* mSpindizzyBlockSet = getElementSet();
  if (mIndependent) {
    mLoadedBlock->createSampleSurfaces();
  } else {
    mSpindizzyBlockSet->registerSurfaceProvider(mLoadedBlock);
    mSpindizzyBlockSet->setDirty();
  }
  return mLoadedBlock;
}

IElementHandler* SpindizzyBlockType::getElementHandler() {
  ISpindizzyBlockSet* mBlockSet = getElementSet();
  return mBlockSet->createHandler();
}

void SpindizzyBlockType::unregisterSurfaces(ISurfaceProcessor* surfaceProcessor) {
  for (unsigned int i = 0; i < cContent.size(); i++) {
    surfaceProcessor->unregisterSurfaceProvider(cContent[i]);
  }
}

BlockTypeProperties* SpindizzyBlockType::getBlockTypeProperties() {
  return &cBlockTypeProperties;
}

void SpindizzyBlockType::save(DOMNodeWriter* node, IResourceLocator* resourceLocator) {
  cBlockTypeProperties.save(node, resourceLocator);
}

void SpindizzyBlockType::setEditingContext(BlockLocation* editingLocation, IComponentContainer* componentContainer) {
  cComponentContainer = componentContainer;
  cEditingLocation = editingLocation;
}

bool SpindizzyBlockType::keyDown(SDLKey& key) {
  switch (key) {
    case SDLK_SPACE: {
      if (cStartBlockLocation == NULL) {
        cStartBlockLocation = new BlockLocation(*cEditingLocation);
      } else {
        SpindizzyBlock* mNewBlock = createBlock(cStartBlockLocation, cEditingLocation, cBlockProperties, true);
    ...
    case SDLK_DELETE: {
      if (cStartBlockLocation == NULL) {
        ...
        SpindizzyBlock* mNewBlock = createBlock(cStartBlockLocation, cEditingLocation, cBlockProperties, false);
        cContent.push_back(mNewBlock);
        addElement(mNewBlock);
        delete cStartBlockLocation;
        cStartBlockLocation = NULL;
...
void SpindizzyBlockType::configureElement() {
...
bool SpindizzyBlockType::input(SDL_Event& event) {
  switch (event.type) {
    case SDL_KEYDOWN: {
...
void SpindizzyBlockType::renderEditingPreview() {
  if (cStartBlockLocation != NULL) {
...
void SpindizzyBlockType::renderIcon() {
  ...
  if (cSampleBlock == NULL) {
    ...
    cSampleBlock = createBlock(&mIdentityBlockLocation, &mIdentityBlockLocation, cBlockProperties, true);
...
void SpindizzyBlockType::componentClosed(IHUDComponent* component) {
  if (component == cConfigurationComponent) {
    cConfigurationComponent = NULL;
  }
}

void SpindizzyBlockType::destroy(IElement* element) {
  delete element;
}

SpindizzyBlock* SpindizzyBlockType::createBlock(BlockLocation* startLocation, BlockLocation* endLocation, SpindizzyBlockProperties* blockProperties, bool addition) {
  return new SpindizzyBlock(this, startLocation, endLocation, blockProperties, addition);
}

void SpindizzyBlockType::resourcePendingDestruction(ITexture* destroyee, ITexture* replacement) {
...
void SpindizzyBlockType::resourceChanged(ITexture* texture) {
...
SpindizzyBlockType::~SpindizzyBlockType() {
  if (cStartBlockLocation != NULL) {
    delete cStartBlockLocation;
  }
  delete cSampleBlock;
