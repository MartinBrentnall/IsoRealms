...
C64SpindizzyTextureSet::C64SpindizzyTextureSet(IRuntimeContext* runtimeContext, const std::string& name) {
  initialise();
  addTextures(runtimeContext, name);
  generateTextures();
}

void C64SpindizzyTextureSet::initialise() {
...
void C64SpindizzyTextureSet::resourceChanged(IColour* colour) {
  if (colour == cBackgroundColour || colour == cFloorColour || colour == cGridColour || colour == cWallColour) {
    generateTextures();
  }
}

void C64SpindizzyTextureSet::resourcePendingDestruction(IColour* destroyee, IColour* replacement) {
  bool mChanged = false;
...
bool C64SpindizzyTextureSet::contains(ITexture* texture) {
  for (std::map<std::string, Texture*>::iterator i = cTextures.begin(); i != cTextures.end(); i++) {
    if (texture == i->second) {
      return true;
    }
...
void C64SpindizzyTextureSet::deleteResources(IResourceInstanceListener<ITexture>* listener) {
  for (std::map<std::string, Texture*>::iterator i = cTextures.begin(); i != cTextures.end(); i++) {
    listener->resourcePendingDestruction(i->second);
  }
}

void C64SpindizzyTextureSet::addTextures(IRuntimeContext* runtimeContext, const std::string& name) {
  for (std::map<std::string, Texture*>::iterator i = cTextures.begin(); i != cTextures.end(); i++) {
...
void C64SpindizzyTextureSet::initialiseResource(DOMNodeWrapper* node, IResourceAccessor* resourceAccessor) {
...
IColour* C64SpindizzyTextureSet::getFloorColour() {
  return cFloorColour;
}

IColour* C64SpindizzyTextureSet::getWallColour() {
  return cWallColour;
}

IColour* C64SpindizzyTextureSet::getGridColour() {
  return cGridColour;
}

IColour* C64SpindizzyTextureSet::getDetailColour() {
  return cBackgroundColour;
}

void C64SpindizzyTextureSet::setFloorColour(IColour* colour) {
  cFloorColour = colour;
  generateTextures();
}

void C64SpindizzyTextureSet::setWallColour(IColour* colour) {
  cWallColour = colour;
  generateTextures();
}

void C64SpindizzyTextureSet::setGridColour(IColour* colour) {
  cGridColour = colour;
  generateTextures();
}

void C64SpindizzyTextureSet::setDetailColour(IColour* colour) {
  cBackgroundColour = colour;
  generateTextures();
}

std::vector<ITexture*> C64SpindizzyTextureSet::getTextures() {
...
void C64SpindizzyTextureSet::save(DOMNodeWriter* node, IResourceLocator* resourceLocator) {
