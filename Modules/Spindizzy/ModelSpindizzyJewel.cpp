...
unsigned int ModelSpindizzyJewel::cReferenceCount = 0;
...
void ModelSpindizzyJewel::refresh() {
  cProgress = (random() % (cColoursCycle->size() * 1000)) / 1000.0f;
...
void ModelSpindizzyJewel::render() {
  glPushMatrix();
  ...
  unsigned int mNext     = mPrevious != cColoursCycle->size() - 1 ? cProgress + 1 : 0;
  float mCurrent         = cProgress - mPrevious;
  Colour mColourPanel((*cColoursCycle)[mPrevious], (*cColoursCycle)[mNext], mCurrent);
  mColourPanel.set();
...
ModelSpindizzyJewel::~ModelSpindizzyJewel() {
  cReferenceCount--;
  if (cReferenceCount == 0) {
...
void ModelSpindizzyJewel::renderInnerTriangle(Vertex& a, Vertex& b, Vertex& c, float lineWidth) {
  Vertex mA;
  Vertex mB;
  Vertex mC;
...
void ModelSpindizzyJewel::renderOuterTriangle(Vertex& a, Vertex& b, Vertex& c, float lineWidth) {
  Vertex mA;
  Vertex mB;
  Vertex mC;
  renderCellTriangle(a, b, c, mA, mB, mC, lineWidth);
...
double ModelSpindizzyJewel::getValue(Vertex a1, Vertex a2, Vertex b1, Vertex b2) {
  return ((b2.x - b1.x) * (a1.y - b1.y) - (b2.y - b1.y) * (a1.x -b1.x)) / ((b2.y - b1.y) * (a2.x - a1.x) - (b2.x - b1.x) * (a2.y - a1.y));
}

double ModelSpindizzyJewel::distance(double a1, double a2, double b1, double b2) {
  double px = a1 - b1;
  double py = a2 - b2;
...
void ModelSpindizzyJewel::renderCellTriangle(Vertex mPointA, Vertex mPointB, Vertex mPointC, Vertex& mResultA, Vertex& mResultB, Vertex& mResultC, float mWidth) {
