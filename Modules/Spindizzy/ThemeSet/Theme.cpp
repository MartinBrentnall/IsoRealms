...
namespace IsoRealms::Spindizzy {
  ...
  std::vector<std::unique_ptr<IProperty>> Theme::getProperties() {
    ...
    for (std::pair<ThemeTexture* const, Texture>& mTexture : cTextures) {
      mProperties.emplace_back(std::make_unique<PropertyAsset<Texture>>(PropertyData(cThemeSet.getElement(mTexture.first), "TODO"), mTexture.second));
    }
    for (std::pair<ThemeColour* const, Colour>& mColour : cColours) {
      mProperties.emplace_back(std::make_unique<PropertyAsset<Colour>>(PropertyData(cThemeSet.getElement(mColour.first), "TODO"), mColour.second));
    }
    return mProperties;
  }
