...
Texture* ModelSpindizzyCraftBall::cTexture = NULL;
unsigned int ModelSpindizzyCraftBall::cInstanceCount = 0;
const float ModelSpindizzyCraftBall::CIRCLE_RESOLUTION = 5.0f * (M_PI / 180.0);

ModelSpindizzyCraftBall::ModelSpindizzyCraftBall(Vertex* location, ICamera* camera) {
  cLocation = location;
  cCamera = camera;
  if (cTexture == NULL) {
...
void ModelSpindizzyCraftBall::update(unsigned int milliseconds) {
  // Nothing to do
}
...
void ModelSpindizzyCraftBall::generateTextureBall() {
...
ModelSpindizzyCraftBall::~ModelSpindizzyCraftBall() {
  if (!(--cInstanceCount)) {
