...
namespace IsoRealms::Spindizzy {
  ...
  SurfacePatternComposite::SurfacePatternComposite(const Metadata& metadata, TerrainType& owner, JSONObject object) :
    ...
    for (JSONValue mPatternValue : object.getArray(JSON_PATTERNS)) {
      ...
      cDefSurfacePatterns.emplace_back(std::make_unique<SurfacePattern>(owner.getSpindizzy(), owner, nullptr)).get()->set(mPatternObject, JSON_PATTERN);
    }
  }
  ...
  void SurfacePatternComposite::getAssetProperties(PropertyMaker& owner) {
    owner.createPropertyArray(cMetadata.getPropertyData("Patterns"), cDefSurfacePatterns, [](const std::unique_ptr<SurfacePattern>& mSurfacePattern)->SurfacePattern& {return *mSurfacePattern;}, [this, &owner](SurfacePattern& surfacePattern) {
      owner.createPropertyAsset<SurfacePattern>(cMetadata.getPropertyData("Pattern"), surfacePattern);
    }, [this]()->SurfacePattern& {
