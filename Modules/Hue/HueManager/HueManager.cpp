...
namespace IsoRealms::Hue {
  const std::string HueManager::ATTRIBUTE_BRIDGE_ADDRESS = "bridge";
  const std::string HueManager::ATTRIBUTE_BRIDGE_USER    = "user";
  const std::string HueManager::ATTRIBUTE_BRIDGE_PSK     = "psk";
  const std::string HueManager::ATTRIBUTE_BULB_ID        = "id";
  const std::string HueManager::ATTRIBUTE_BULB_COLOUR    = "colour";

  const std::string HueManager::TAG_BULB = "Bulb";
  ...
  HueManager::HueManager(IProject* project, Hue* hue) {
  ...
  HueManager::HueManager(IProject* project, Hue* hue, DOMNode& node, IOptions* options, IResourceData* data) :
            HueManager(project, hue) {
    cDefBridgeAddress = node.getAttribute(ATTRIBUTE_BRIDGE_ADDRESS);
    cDefBridgeUser    = node.getAttribute(ATTRIBUTE_BRIDGE_USER);
    cDefBridgePSK     = node.getAttribute(ATTRIBUTE_BRIDGE_PSK);

    int mEntertainmentBulbID = 0;
    for (DOMNode& mChild : node) {
      std::string mChildName = mChild.getName();
      if (mChildName == TAG_BULB) {
        cDefBulbs.emplace_back(std::make_unique<Bulb>(this, mChild, project, mEntertainmentBulbID));
        mEntertainmentBulbID++;
      } else {
  ...
  std::vector<IProperty*> HueManager::getProperties(IAssetBrowser* browser, IAssetRegistry* assets, IPropertyListener* listener) {
  ...
  void HueManager::registerAssets(IAssetRegistry* assets) {
    // Nothing to do.
  }

  void HueManager::unregisterAssets(IAssetRemover* assets, IAssets* releaser) {
    // Nothing to do.
  }
  
  void HueManager::save(DOMNodeWriter* node, IAssetIdentifier* identifier) const {
...
//  void HueManager::relinquish(IActionType* asset) {
//    // FIXME:TripPlayer: Implement this
//  }
  ...
  HueManager::Bulb::Bulb(HueManager* parent, DOMNode& node, IProject* project, int id) :
            cParent(parent),
            cDefID(id),
  ...
  int HueManager::REST::configureCURL(RequestType requestType, const std::string& url) {
    ...
    if (cCurl) {
      ...
      if (requestType == RequestType::PUT) {
        ...
        curl_easy_setopt(cCurl, CURLOPT_PUT, 1L);
