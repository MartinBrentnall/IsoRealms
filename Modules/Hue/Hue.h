...
namespace IsoRealms::Hue {
  class Hue : public IModuleHandle {
    private:
    static const std::string TAG_RESOURCE_HUE_MANAGER;

    static const std::string NAME_RESOURCE_HUE_MANAGER;
    
    ResourceTypeDefinition<Hue, HueManager> cResourceTypeHueManager;
      
    public:
    Hue(IProject* project, IResourceTypeRegistry* registry, IAssetLiterals* literals);
