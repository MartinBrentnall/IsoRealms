...
namespace IsoRealms::Equilibria {
  ...
  void CameraGameplay::publish(ResourcePublisher& publisher, const std::string& parentID) {
    cStateNotifier = publisher.publish<IFloat>(this,         parentID + "/ViewYaw",   "Gameplay Cameras");
    publisher.publish<IFloat>(                 &cPitch,      parentID + "/ViewPitch", "Gameplay Cameras");
    publisher.publish<IBinding>(               &cLuaBinding, parentID,                "Gameplay Cameras");
  }
  ...
  void CameraGameplay::reset() {
    ...
    cCachedXZoom = (std::abs(std::sin(Equilibria::DEFAULT_VIEW_ANGLE_YAW * (M_PI / 180.0f)) * mXSize) + std::abs(std::cos(Equilibria::DEFAULT_VIEW_ANGLE_YAW * (M_PI / 180.0f)) * mYSize)) / 2.0f;
    cCachedYZoom = (std::abs(std::sin(Equilibria::DEFAULT_VIEW_ANGLE_PITCH * (M_PI / 180.0f)) * (mZSize / 2.0f)) + std::abs(std::cos(Equilibria::DEFAULT_VIEW_ANGLE_PITCH * (M_PI / 180.0f)) * cCachedXZoom));
  }
