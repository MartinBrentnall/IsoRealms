...
namespace IsoRealms::Equilibria {
  ...
  Zone::Zone(World& world, JSONObject object) :
    ...
    cDefWorld.getEquilibria().getProject().init([this, object]() {
      cDefType = cDefWorld.getEquilibria().get<ZoneType>(nullptr, object.getString(JSON_TYPE));
      std::string mThemeSet = object.getString(JSON_THEME_SET);
      std::string mThemeName = object.getString(JSON_THEME);
      cDefThemeSet = mThemeSet != "" ? cDefWorld.getEquilibria().get<ThemeSet>(nullptr, mThemeSet) : nullptr;
  ...
  void Zone::addTerrainState(Terrain* terrain) {
    ...
    if (mCondition.has_value()) {
      ...
      for (std::set<IBoolean*>::iterator i = mInputs.begin(); i != mInputs.end(); i++) {
        TerrainState* mTerrainState = cDefWorld.getEquilibria().getTerrainState(*i);
        bool mExists = false;
...
//    mZoneVisuals.push_back(this);
  ...
  void Zone::equilibriaZoneThemeEdited(Theme* theme) {
    if (cDefTheme == theme) {
      updateDisplayList();
    }
  }
  ...
  bool Zone::isValidTerrainPlacement(const WorldEditorCursorCell& start, const WorldEditorCursorCell& end, int southWestHeight, int southEastHeight, int northWestHeight, int northEastHeight) const {
    int mXSlope = Equilibria::getTerrainSlope(southWestHeight, northWestHeight, southEastHeight, northEastHeight);
    int mYSlope = Equilibria::getTerrainSlope(southWestHeight, southEastHeight, northWestHeight, northEastHeight);
  ...
  void Zone::bindValues() {
    cDefWorld.getEquilibria().bind1(this);
  ...
  void Zone::bindValues2(Wall* wall) {
    cDefWorld.getEquilibria().bind2(this);
    cDefWorld.getEquilibria().bind(wall);
    for (IZoneProperty* mProperty : cRuntimeZoneProperties) {
  ...
  void Zone::unbindValues() {
    cDefWorld.getEquilibria().bind1(static_cast<Zone*>(nullptr));
    cDefWorld.getEquilibria().bind(static_cast<Wall*>(nullptr));
    for (IZoneProperty* mProperty : cRuntimeZoneProperties) {
  ...
  void Zone::unbindValues2() {
    cDefWorld.getEquilibria().bind2(static_cast<Zone*>(nullptr));
    for (IZoneProperty* mProperty : cRuntimeZoneProperties) {
