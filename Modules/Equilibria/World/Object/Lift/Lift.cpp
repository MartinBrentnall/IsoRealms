...
#include "Modules/Equilibria/LiftType/LiftType.h"
...
namespace IsoRealms::Equilibria {
  ...
  Lift::Lift(Zone& zone, JSONObject object) :
    ...
    cZone.getWorld().getEquilibria().getProject().init([this, object]() {
      cDefType = cZone.getWorld().getEquilibria().get<LiftType>(nullptr, object.getString(JSON_TYPE));
      cDefModel = cDefType->createModel();
      reset();
    });
  }
  ...
  void Lift::updateRuntime(unsigned int milliseconds) {
    ...
    if (cDefType->isActive() && !cZone.getWorld().getEquilibria().isPaused()) {
...
//       std::cout << "SETTING GRADIENT: " << mGradient << " (" << mEvent->cTime << " / " << mDuration << ")" << std::endl;
  ...
  void Lift::getProperties(IPropertyMaker& owner) {
    const Metadata& mMetadata = cZone.getWorld().getEquilibria().getMetadata("Lift");
