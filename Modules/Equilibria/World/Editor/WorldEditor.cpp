...
namespace IsoRealms::Equilibria {
  WorldEditor::WorldEditor(Project& assets, World& world, IDialogManager& dialogManager) :
            ...
            cScreenPitch(&cTilt),
            cHatHandler(world.getEquilibria().getProject().getApplication().getHatHandler()),
  ...
  void WorldEditor::setNextTheme() {
    ...
    if (mZone != nullptr) {
      ...
      cWorld.getEquilibria().setNextTheme();
    }
  }
  ...
  void WorldEditor::setPreviousTheme() {
    ...
    if (mZone != nullptr) {
      ...
      cWorld.getEquilibria().setPreviousTheme();
    }
  }
  ...
  void WorldEditor::renderScreen(float scale, float aspectRatio) const {
    cWorld.getEquilibria().applyDefaultThemes();
    glPushMatrix();
  ...
  void WorldEditor::notifyHidden() {
    cWorld.getEquilibria().setAllThemesInUse(false);
  }
