...
ModelIcon::ModelIcon(IResourceBrowser<I3DModelType>* browser, I3DModelType* modelType) : Icon<I3DModelType>(browser, modelType) {
  c3DModelType = modelType;
}

void ModelIcon::renderIcon() {
  glPushMatrix();
  float mScale = 0.06f;
  ...
  c3DModelType->renderIcon();
  glPopMatrix();
}

void ModelIcon::updateIcon(unsigned int milliseconds) {
  c3DModelType->updateIcon(milliseconds);
}

float ModelIcon::getWidth() {
  Configuration* mConfiguration = Configuration::getInstance();
