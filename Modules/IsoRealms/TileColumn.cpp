...
bool TileColumn::isTileVisible(IGeometricElement* geometricElement, ITileSurface::FaceDirection facing) {
  for (unsigned int i = 0; i < cTileBlocks.size(); i++) {
    if (cTileBlocks[i]->getGeometricElement(facing) == geometricElement) {
      return true;
    }
  }
  for (unsigned int i = 0; i < cGhostTileBlocks.size(); i++) {
    if (cGhostTileBlocks[i]->getGeometricElement(facing) == geometricElement) {
      return true;
    }
  }
  return false;
