...
namespace IsoRealms::UI {
  ...
  std::vector<std::unique_ptr<IProperty>> LayoutEditor::getAssetProperties() {
  ...
  void LayoutEditor::setExitAction(ActionExecutor* action) {
    cExitAction = action;
  }
  ...
  bool LayoutEditor::CopyTool::input(sf::Event& event) {
    switch (event.type) {
      case sf::Event::JoystickButtonPressed: {
        switch (event.joystickButton.button) {
          case 0: {
            LayoutComponent* mComponentCopy = cParent.cLayout.createComponent(cParent.cSelectedComponent);
