...
namespace IsoRealms {
  ...
  class ScreenClientManager {
    ...
    public:
    ScreenClientManager(ILiteralAssetProvider<IResourceData, IScreen>* literal, Project& project) :
              cProject(project),
              cScreens(literal) {
    }
    ...
    IScreen* getProxy(IScreen* screen) {
      std::map<IScreen*, std::unique_ptr<ScreenProxy>>::iterator mProxy = cScreenProxyMapping.find(screen);
    ...
    IStateNotifier<IScreen>* add(IAssetProvider<IResourceData, IScreen>* provider, const std::string& id, const std::string& category, bool stateChanges = false) {
      return cScreens.add(provider, id, category, stateChanges);
    }
    ...
    IScreen* getDefault(IAssetUser<IScreen>* user, IResourceData& owner) {
      return cScreens.getDefault(user, owner);
    }
    ...
    bool hasReadOnlyReferences(IScreen* asset) const {
      std::map<IScreen*, std::unique_ptr<ScreenProxy>>::const_iterator mProxy = cScreenProxyMapping.find(asset);
      if (mProxy == cScreenProxyMapping.end()) {
        throw ArgumentException("ERROR: Project::remove: Proxy for specified screen asset not found.");
      }
    ...
    bool hasReadOnlyReferences(IAssetProvider<IResourceData, IScreen>* provider) const {
