...
#include "IsoRealms/Resources/Client/BindingType.h"
...
namespace IsoRealms {
  ...
  Binding::Binding(IActionContext& owner, const BindingType* type) :
            Resource<Binding, IBinding, IActionContext>(owner),
            cDefType(type),
  ...
  TreeItemInfo Binding::getTreeItemInfo() const {
    if (cDefRegistry != nullptr) {
      std::string mBindingID = cDefRegistry->getBindingID(cResource);
      if (!mBindingID.empty()) {
    ...
    TreeItemInfo mTreeItemInfo = Resource::getTreeItemInfo();
    std::string mConversionPath = cResource->getConversionPath();
  ...
  bool Binding::renderTreeItemIcon(const std::string& id) const {
    ...
    return Resource::renderTreeItemIcon(id);
  }
  ...
  void Binding::forEachAvailableTreeItem(std::function<void(const TreeItemInfo&)> getTreeItemInfoFunction) const {
    ...
    if (mType.empty()) {
      if (cDefRegistry != nullptr) {
        cManager.getResourceManager().forEachEntry<IBinding>([&getTreeItemInfoFunction](const TreeItemInfo& mTreeItemInfo) {
          getTreeItemInfoFunction(TreeItemInfo{mTreeItemInfo.cID, "Global/" + mTreeItemInfo.cPath});
        });
      } else {
        cManager.getResourceManager().forEachEntry<IBinding>(getTreeItemInfoFunction);
      }
      return;
    }

    // Case where only a specific type is allowed.
    cManager.getResourceManager().forEachEntry<IBinding>([&getTreeItemInfoFunction, &mType, this](const TreeItemInfo& mTreeItemInfo) {
