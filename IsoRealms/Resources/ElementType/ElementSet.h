/*
 * Copyright 2009,2010 Martin Brentnall
 *
 * This file is part of Iso-Realms.
 *
 * Iso-Realms is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * Iso-Realms is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with Iso-Realms.  If not, see <http://www.gnu.org/licenses/>.
 */
#ifndef ELEMENT_SET_H
#define ELEMENT_SET_H

#include <map>

#include <IsoRealms/DefaultElementHandler.h>
#include <IsoRealms/IPlugin.h>

template<class T = IElement> class ElementSet:public IPlugin {
  private:
//   std::map<IElementContainer*, DefaultElementHandler<T>*> cHandlers;

  DefaultElementHandler<T>* getElementHandler(/*IElementContainer* elementContainer*/) {
//     DefaultElementHandler<T>* mHandler = cHandlers[elementContainer];
//     if (mHandler == NULL) {
//       mHandler = createHandler(elementContainer);
//       cHandlers[elementContainer] = mHandler;
//       elementContainer->addElementHandler(mHandler);
//     }
//     return mHandler;
  }

  public:
  void addElement(T* element/*, IElementContainer* elementContainer*/) {
//     DefaultElementHandler<T>* mHandler = getElementHandler(elementContainer);
//     mHandler->addElement(element);
  }

  void removingElement(IElement* element/*, IElementContainer* elementContainer*/) {
//     DefaultElementHandler<T>* mHandler = getElementHandler(elementContainer);
//     mHandler->removeElement(element);
  }
  
  void addingElement(IElement* element/*, IElementContainer* elementContainer*/) {
//     DefaultElementHandler<T>* mHandler = getElementHandler(elementContainer);
//     mHandler->addElement(element);
  }
  
  virtual DefaultElementHandler<T>* createHandler(/*IElementContainer* elementContainer*/) {
    return new DefaultElementHandler<T>();
  }
};

#endif
