...
namespace IsoRealms {
  class PropertyColourSaturation : public PropertySlider {
    ...
    public:
    PropertyColourSaturation(PropertyMaker& owner, const PropertyData& data, std::function<float()> valueFunction, float* hue, float* lightness, float* alpha, std::function<void(const float)> confirmationCallback);
