...
namespace IsoRealms {
  ...
  class IPropertyMaker : public IResourceAccessManager {
    ...
    virtual void createPropertyOptional(             const PropertyData& metadata, IOptionalObject& optionalSource, const std::string& noneLabel, std::function<bool()> noneIcon, std::function<void(const std::string&)> choiceCallback, std::function<std::string()> valueGetter = nullptr) = 0;
